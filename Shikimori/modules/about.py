...
try:
    if network_name == "macnetwork7":
        HMMM = InlineKeyboardButton(text="MAC NETWORK", callback_data="sern_")
    elif NETWORK:
    ...
    if query.data == "github_":
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                    InlineKeyboardButton(text="Repo", url="Only For Devs"),
                    InlineKeyboardButton(text="Creator", url="https://github.com/definitelynotchirag"),
                    ],
                ]
            ),
        )
...
def sern_call_back(update: Update, context: CallbackContext):
    ...
    if query.data == "sern_":
        query.message.edit_text(
            text=f"""
ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ [MAC 𝘕𝘌𝘛𝘞𝘖𝘙𝘒](https://t.me/macnetwork7/12),
MAC 𝙞𝙨 𝙖𝙣 𝙖𝙣𝙞𝙢𝙚 𝙗𝙖𝙨𝙚𝙙 𝘾𝙤𝙢𝙢𝙪𝙣𝙞𝙩𝙮 𝙬𝙞𝙩𝙝 𝙖 𝙢𝙤𝙩𝙞𝙫𝙚 𝙩𝙤 𝙨𝙥𝙧𝙚𝙖𝙙 𝙡𝙤𝙫𝙚 𝙖𝙣𝙙 𝙥𝙚𝙖𝙘𝙚 𝙖𝙧𝙤𝙪𝙣𝙙 𝙩𝙚𝙡𝙚𝙜𝙧𝙖𝙢. Its a network under which inter-connects all the groups and channel we have created or manage.""", parse_mode=ParseMode.MARKDOWN,
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    ...
                    InlineKeyboardButton(text="★彡[ᴏꜰꜰɪᴄɪᴀʟ ɢʀᴏᴜᴘ]彡★", url="https://t.me/AnimeChattingGroup")
                    ],
                ]
            ),
        )
