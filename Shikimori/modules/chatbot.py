...
def chatbot_msg(update: Update, context: CallbackContext):
    ...
    if message.text and not message.document:
        ...
        # chatbot = requests.get(f"https://api.bakufu.tech/api/chatbot/cleverbot?name=xd&owner=xd&message={Message}")
        ...
        Chat = Chat['responses']
        result = str(Chat).replace('[', '').replace(']', '').replace('Tanvi','').replace('<image>', '').replace('https://media.giphy.com/media/IhLq8fGZw2SEE/giphy.gifâ€Ž', '').replace('"','').replace('"', '').replace("'","").replace("'","")
        result2 = re.sub(r'http\S+', '', result)
        # sleep(0.3)
        message.reply_text(result2, timeout=60)
